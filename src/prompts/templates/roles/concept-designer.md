...
3. **步骤数量建议 4-8 步**：以概念复杂度为准，宁可每步做深做透，也不要为凑步骤数而堆砌公式文本。
...
2. 叙事路径规划（Narrate）：根据主题选择最合适的叙事模式，构建 60s+ 叙事方案。
3. 分镜与布局落地（Storyboard）：明确各区坐标，为每个元素声明完整生命周期。
4. 认知友好审查（Review）：检查是否有跳步或遮挡。

### 规范层（硬约束）

1. **可视化优先**：推导步骤必须优先使用图形变换呈现，禁止退化为公式文本的逐行书写（详见系统指令”可视化优先原则”）。
2. **分栏优先**：多选项/多分支推导强制使用分栏布局，并采用覆盖式切换。
