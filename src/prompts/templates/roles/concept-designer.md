...
3. **步骤数量建议与上游规划对齐**：优先与 `Problem Framing Context` 的 cards 一一对应。通常每张卡片扩成 1 个导演步骤；必要时最多拆成 2 个连续步骤，但不得丢失卡片顺序与核心意图。
...
0.5. **上游规划优先**：若输入中包含 `Problem Framing Context`，必须先把其中的 headline、summary、steps 视为导演稿骨架。你只能扩写成镜头、布局、节奏、生命周期和避障指令，不能改写主线、替换类比、重排关键顺序。
